...
def validate_data_origin(df, prediction_type="classification", target_column=None, categories=None, ordinals=None):
    ...
    if prediction_type not in ["classification", "regression"]:
        logger.warning(f"Invalid prediction type: '{prediction_type}'")
    ...
    if df.shape[0] < MIN_ROWS:
        logger.warning("Dataset has dimensions {}, classification datasets must have at least {} rows.".format(
            df.shape, MIN_ROWS))
        return False, "Dataset has dimensions {}, classification datasets must have at least {} rows.".format(df.shape, MIN_ROWS)

    if df.shape[1] < MIN_COLS:
        logger.warning("Dataset has dimensions {}, classification datasets must have at least {} columns.".format(
            df.shape, MIN_COLS))
    ...
    if categories or ordinals:
        try:
            ...
            logger.warning("encode_data() failed, " + str(e))
    ...
    if target_column:
        ...
        if (len(num_df.columns)) > 0:
            try:
                ...
                logger.warning("sklearn.check_array() validation " + str(e))
    ...
    if target_column:
        ...
        if (len(df.columns)) > 0:
            try:
                ...
                logger.warning("check_dataframe() validation " + str(e))
